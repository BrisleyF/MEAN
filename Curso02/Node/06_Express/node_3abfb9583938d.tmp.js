...
//
// Asignando el response code
//
app.post("/coches", insertarCoche)

function insertarCoche(request, response){

    //let cocheInsertado = negocioCoches.insertar(bla bla blá)
    let cocheInsertado = {
            _id    : 1234,
            marca  : "SEAT",
            modelo : "Panda 45"
    ...
    //response.json(cocheInsertado)
    
    //response.status(201) 
    //response.json(cocheInsertado)

    //Tambien han implementado 'fluent api'
    response
        .status(201)
        .json(cocheInsertado)
}
...
//Debemos añadir este middleware antes que las rutas de nuestro api
app.use(express.json({
    limit: '5mb' //Tamaño máximo del body que estamos dispuestos a leer. IMPRESCINDIBLE
}))

//Si fueramos a recibir también parámetros en el body (por ejemplo del submit de un <form>)
